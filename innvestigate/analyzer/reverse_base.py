    ...
    * :func:`_head_mapping` defines how the outputs of the model
    ...
    def _gradient_reverse_mapping(
        ...
        if self.masks is not None:
            ...
            percentage_change = reverse_state["percentage_change"]
            ...
            contribution = ilayers.Split(num_or_size_splits=reverse_state["masks_size"], axis=0)(contribution)
            contribution = [ilayers.Divide_no_nan()([a, contribution[-1]]) for a in contribution]
            
            contribution = [ilayers.Multiply()([a, tf.squeeze(Ys[0])]) for a in contribution]

            # contribution = [ilayers.Multiply()([a, b]) for a, b in zip(max_activ, reversed_Ys)]
            contribution = [ilayers.Concat(axis=0)(contribution)]
            # contribution = [ilayers.Multiply()([,Ys[0]])]
            return contribution
